...
CSV_HEADERS = [
    ...
    # "url",            # required for testing no-cache policy
    # "resolved_ip"     # if hosting changes across CDNs
]
...
def bust_cache(url: str) -> str:
    ...
    params = parse_qs(parsed_url, keep_blank_values=True)
    params["_cbt"] = uuid.uuid4().hex       # cache bust token added to query string
    new_query = urlencode(params, doseq=True)
    return urlunparse(parsed_url._replace(parsed_url.query, new_query))



def stream_download(url: str, session: requests.Session) -> dict:
    headers = {
    ...
    # compute throughput in Mbps; note that elapsed time is in milliseconds
    throughput = (total_bytes * 8)/(elapsed_time * 1e6) if elapsed_time > 0 else 0.0

    return {
        "http_status": response.status_code,
        "elapsed_time_s": (elapsed_time * 1e6),
        "filesize_bytes": total_bytes,
        "throughput_Mbps": throughput,
        "final_url": response.url     # urls might differ if server-side redirects are involved
    }
